...
multicollinearity_matrix = cor(TrainData[-2]) #Discarding 1st column while checking ulticollinearitty
...
log_reg_2 = ctree(formula = click~., data=TrainData) #glm = generlaized linera model
log_reg_2
...
test_pred_class = factor(ifelse(test=test_set_prob >0.5, yes = 1, no = 0))
names(test_pred_class)

table(TestData$click, test_pred_class)
model_accuracy = (17382+7744)/(17382+7744+1202+24206); model_accuracy
